...
public class SelectActionDP extends JosmAction implements DataSelectionListener {
    private final GeometryFactory gf = new GeometryFactory();
    private final CommonUtils commonUtils = new CommonUtils();


    public SelectActionDP() {
        super(tr("dp"), "mapmode/magic-wand-merge", tr("hull nodes"), Shortcut.registerShortcut("data:magicwanddp", tr("Data: {0}", tr("dp nodes")), KeyEvent.VK_6, Shortcut.CTRL), true);
    }
    ...
    private List<Polygon> mergeWays(Collection<Way> ways) {
        ...
        for (Way w : ways) {
            ...
            var geom_filter = (Polygon) commonUtils.simplifyDP(geom, ToolSettings.getSimplDP());
            System.out.println("original " + tmpCoords.size() + " simplifyDP " + geom_filter.getCoordinates().length);
    ...
    private boolean isdrawWays(List<Polygon> polygonsMerge) {
        ...
        for (Polygon pol : polygonsMerge) {
            ...
            w.setKeys(new TagMap("dp", Double.toString(ToolSettings.getSimplDP())));
